...
OptionMultiFlux ::OptionMultiFlux(nlohmann::json &jsonParams) {
...
double OptionMultiFlux ::payoff(const PnlMat *path, double interestRate) {
    bool C = true;
    double payoff = 0.0;
    ...
    for (size_t index = 0; index < nAssets_; index++) {
        ...
        payoff += capital * std::max(MGET(path, index+1, index)- GET(strikes_, index), 0.0) * (int)C;
        C = C && (MGET(path, index + 1, index) <= GET(strikes_, index));
        if (!C) break;
    }
    return payoff;
}
